...
for d in ${DIM[@]}; do
    ...
    for i in `seq 1 $ITERATIONS`; do
        ...
        #ITERATION=$(bin/celldevs_benchmark_sequential $d $TIME)
        #echo $ITERATION
    ...
    for i in `seq 1 $ITERATIONS`; do
        ...
        #ITERATION=$(bin/celldevs_benchmark_naive_parallel $d $TIME 32)
        #echo $ITERATION
    ...
    for t in ${THREADS[@]}; do
        ...
        for i in `seq 1 $ITERATIONS`; do
           ...
           #ITERATION=$(bin/celldevs_benchmark_static_parallel_numa $d $TIME $t)
           #echo $ITERATION
