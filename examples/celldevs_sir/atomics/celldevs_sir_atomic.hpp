...
#ifndef CELL_DEVS_SIR_ATOMIC
#define CELL_DEVS_SIR_ATOMIC
...
#include <cmath>
    ...
    class CellDEVSSirAtomic {
     public:
    	...
    	CellDEVSSirAtomic(double p, double s, double i, double r){
		...
		SIRState localComputation(SIRState state, SIRState neighborhood[9]) {
		...
		double newInfections(SIRState& state, SIRState neighborhood[9]) {
			double aux = 0;
		...
		double newRecoveries(SIRState& state){
			return state.i * rec;
		}
...
*/
		...
		* @return the sigma value.
		*/
		void insert_in_bag(SIRState in_message) {
